    ...
    <div class="bg-gray-50 min-h-screen py-12 flex flex-col justify-center sm:px-6 lg:px-8" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            ...
            <h2 class="mt-2 text-center text-3xl font-extrabold text-gray-900 leading-tight">
                {{ app()->getLocale() == 'ar' ? 'المطالبة بحسابك' : 'Claim Your Profile' }}
            </h2>
            <p class="mt-2 text-center text-sm text-gray-600">
                {{ app()->getLocale() == 'ar' ? 'أدخل البريد الإلكتروني الذي سجلت به مسبقاً.' : 'Enter the email address associated with your existing membership.' }}
            </p>
        </div>
        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-white py-8 px-4 shadow-sm sm:rounded-2xl border border-gray-100 sm:px-10">
                ...
                <form class="space-y-6" action="{{ route('claim.profile.verify') }}" method="POST">
                    @csrf
                    
                    <!-- Email -->
                    <div>
                        <label for="email" class="block text-sm font-bold text-gray-700">
                            {{ app()->getLocale() == 'ar' ? 'البريد الإلكتروني المسجل لدينا' : 'Registered Email Address' }}
                        </label>
                        <div class="mt-1">
