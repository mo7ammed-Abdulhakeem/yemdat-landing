<?php

return [
    'title' => 'نبني مستقبل البيانات معاً',
    'subtitle' => 'انضم إلى شبكة متنامية من المحترفين والمنظمات المكرسة لتطوير إدارة البيانات والذكاء الاصطناعي في اليمن.',
    'register_now' => 'سجل الآن',

    'types_title' => 'أنواع العضويات',

    'individual_title' => 'عضوية متدرب',
    'individual_desc' => 'للطلاب والخريجين الجدد والأفراد المهتمين ببدء مسيرتهم في البيانات.',

    'expert_title' => 'عضوية الخبراء والأكاديميين',
    'expert_desc' => 'للخبراء والأكاديميين المتخصصين.',

    'corporate_title' => 'عضوية مؤسسية للجهات الداعمة',
    'corporate_desc' => 'للمؤسسات والشركات الداعمة.',

    'benefits_title' => 'مزايا العضوية',
    'benefit_1' => 'الوصول إلى ورش عمل متخصصة',
    'benefit_2' => 'التواصل مع قادة المجال',
    'benefit_3' => 'المشاركة في الفعاليات السنوية',
    'benefit_4' => 'المساهمة في معايير البيانات الوطنية',

    'free_note_1' => 'لا توجد أي رسوم مالية',
    'free_note_2' => 'مجتمع مهني تطوعي',

    // Registration Form
    'form_title' => 'نموذج التسجيل',
    'form_subtitle' => 'املأ النموذج للانضمام إلى مجتمع YEMDAT',
    'label_fullname' => 'الاسم الكامل',
    'placeholder_fullname' => 'أدخل اسمك الكامل',
    'label_email' => 'البريد الإلكتروني',
    'placeholder_email' => 'example@email.com',
    'label_phone' => 'رقم الهاتف',
    'placeholder_phone' => 'xxx xxx xxx',
    'label_country' => 'الدولة',
    'select_country' => 'اختر الدولة',
    'label_speciality' => 'التخصص الجامعي',
    'select_speciality' => 'اختر التخصص',
    'specialty_other' => 'تخصص آخر',
    'specialty_placeholder' => 'يرجى تحديد تخصصك',

    'education_level' => 'المستوى التعليمي',
    'edu_high_school' => 'ثانوية عامة',
    'edu_bachelor' => 'بكالوريوس',
    'edu_master' => 'ماجستير',
    'edu_phd' => 'دكتوراه',
    'edu_other' => 'أخرى',

    'label_membership_type' => 'نوع العضوية',
    'select_membership_type' => 'اختر نوع العضوية',
    'btn_submit' => 'تسجيل',
    'btn_cancel' => 'إلغاء',

    // Options
    'country_yemen' => 'اليمن',
    'country_saudi' => 'السعودية',
    'country_uae' => 'الإمارات',
    'country_egypt' => 'مصر',
    'country_other' => 'أخرى',

    'spec_data_mgmt' => 'إدارة البيانات',
    'spec_data_management' => 'إدارة البيانات',
    'spec_governance' => 'حوكمة البيانات',
    'spec_data_governance' => 'حوكمة البيانات',
    'spec_ai' => 'الذكاء الاصطناعي',
    'spec_artificial_intelligence' => 'الذكاء الاصطناعي',
    'spec_analytics' => 'تحليل البيانات',
    'spec_data_analytics' => 'تحليل البيانات',
    'spec_other' => 'أخرى',

    'type_individual' => 'عضوية متدرب',
    'type_expert' => 'عضوية الخبراء',
    'type_corporate' => 'عضوية مؤسسية',
];
